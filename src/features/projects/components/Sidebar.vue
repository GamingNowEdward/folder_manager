...
<template>
  <div class="sidebar">
    ...
    >
      <span class="item-dot" :class="{ active: project.id === currentId }"></span>
      {{ project.name }}
    </div>
    <div v-if="projects.length === 0" class="sidebar-empty">暂无项目</div>

    <div class="sidebar-actions">
      <div class="sidebar-item" @click="emit('add')">
        <span class="action-icon">＋</span>新建项目
      </div>
      <div
        class="sidebar-item"
        :class="{ disabled: !currentId }"
        @click="currentId && emit('edit')"
      >
