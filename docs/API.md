...
`revision` 是配置的修订号，每次配置被修改（UI 或 API）都会 +1。
Agent 可以低成本轮询 `/api/health`，只在 `revision` 变化时才重新拉取 `/api/projects`。
...
按 id 把文件夹从项目中移除。**只删除 `config.json` 中的记录。**
...
| 状态码 | 含义 |
| --- | --- |
| 200 OK | 查询成功；添加已存在的文件夹（幂等命中） |
| 201 Created | 成功创建项目 / 文件夹 |
| 400 Bad Request | 参数错误：非法 id、路径不存在/不是目录/非绝对路径、JSON 语法错误 |
...
| `FOLDER_NOT_FOUND` | 404 | 项目中没有该文件夹 |
...
2. **无认证 Token**：同机任意进程（与本应用同一 Windows 用户）都可以调用。
   这是本地桌面工具 API 的常规取舍；如果需要更强隔离，请通过 Windows 防火墙/用户账户隔离，
   或把应用运行在专用账户下。
3. **不能删除真实文件夹**：`DELETE` 只修改 `config.json`；
   代码里不存在 `remove_dir_all` / `remove_file` 之类的调用
   （见 `src-tauri/src/application/config_service.rs`，删除只做 `Vec::remove`）。
4. **不是文件读写接口**：
   - API 只返回 Folder Manager 自己管理的项目/文件夹元数据；
   - 无法通过任何参数读取文件内容、列目录、写文件；
...
5. **路径处理**：
   ...
   - 长度上限 4096 字符。
6. **id 严格校验**：`project_id` / `folder_id` 长度 1..=128，且只允许 `[A-Za-z0-9_-]`。
7. **请求体上限** 64 KiB，避免超大 payload。
8. **日志**：只记录出错的 HTTP 方法、路径与状态码；不记录 header、token 与请求体。
...
1. 先 `GET /api` 看能力，再 `GET /api/projects` 拿 `id`。
2. 记住 `id`，不要依赖数组顺序。
3. 添加文件夹前不必先查询：本接口幂等，重复调用返回 200 与已存在项。
4. 需要轮询时用 `GET /api/health` 的 `revision` 字段，避免反复拉取全量数据。
5. 只使用 API 返回的 `id` 作为路径参数，不要自己拼 id。
6. 错误处理：读取 `error.code` 做分支（稳定），`error.message` 只用于展示/日志。
